...
class ForgotController: UIViewController {
    ...
    @IBAction func getAuthCode(_ sender: Any) {
        ...
        // 使用异步请求
        Http.post(UrlConstants.AUTH_CODE, params: ["mobile": username, "type": "FORGOT"], callback: authCodeCallback)
    }
    ...
    @IBAction func submit(_ sender: Any) {
        ...
        // 使用异步请求
        Http.post(UrlConstants.FORGOT, params: ["username": username, "password":password, "authCode": authCode], callback: forgotCallback)
    }
    ...
    func forgotCallback(res: HTTPResult) {
        ...
        if result.0 {
            // 跳转到找回密码成功界面
            DispatchQueue.main.async {
                let vc = self.storyboard?.instantiateViewController(withIdentifier: "ForgotSuccessController");
